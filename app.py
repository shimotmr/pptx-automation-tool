...
STATE_DB_FILE = "resume_state_db.json"       # per filename: resume pipeline state
...
def load_resume_db():
    return _read_json(STATE_DB_FILE, {})


def save_resume_db(db):
    _write_json(STATE_DB_FILE, db)
...
def scroll_to_bottom():
    # Best-effort: scroll main container
    components.html(
        """
<script>
  try{
...
# =========================================================
#     Google Sheet Category Options (from Presentations!B)
# =========================================================
def get_category_options(bot: PPTAutomationBot):
    # fallback list (won't break UI if sheet not accessible)
    fallback = ["清潔", "配送", "購物", "AURO"]
    try:
        if not getattr(bot, "sheets_service", None):
            return fallback
        # Presentations!B:B is Category col (per ppt_processor.log_to_sheets)
        resp = bot.sheets_service.spreadsheets().values().get(
            spreadsheetId=bot.SPREADSHEET_ID if hasattr(bot, "SPREADSHEET_ID") else None,
        ...
        # Some deployments won't expose SPREADSHEET_ID attribute; try constant import fallback
        try:
...
# =========================================================
#           Resume / Breakpoint State (per file)
# =========================================================
...
def load_pipeline_state(file_name: str):
    p = state_paths(file_name)["state_json"]
    return _read_json(p, {})
...
def safe_replace_videos(bot: PPTAutomationBot, source_path: str, mod_path: str, video_map: dict, progress_callback=None):
    """
    - If no videos detected => just copy source -> mod_path (keep pipeline consistent)
    - If videos exist => call bot.replace_videos_with_images with required args
    """
    ...
    # normal path
    # Some older bot versions require (source, out, video_map)
...
def execute_automation_logic(bot: PPTAutomationBot, source_path: str, file_name: str, file_prefix: str, jobs: list[dict], auto_clean: bool):
    ...
    Pipeline (with resume):
      S0 source.pptx ready
      S1 extract/upload videos => video_map.json
      S2 replace videos with images => modified.pptx
      S3 shrink => slim.pptx   (needs percent numbers)
      S4 split & upload => split_results.json
      S5 embed videos => final_results.json
      S6 log to sheets
    """
    ensure_workspace()
    paths = state_paths(file_name)
    ...
    # percent callbacks (same line changes)
    ...
    def log_cb(msg: str):
        # keep logs server-side (avoid emoji / avoid noisy UI)
        print(f"[APP] {msg}")

    # --- Resume detection inside execution ---
    src_fp = file_fingerprint(source_path)
    # persist fingerprint for resume validation
    try:
    ...
    start_from = "videos"
    if can_resume:
        # Resume from next stage after what's completed
        if stage == "videos_done":
            start_from = "replace"
        elif stage == "replace_done":
            start_from = "shrink"
    ...
    try:
        ...
        # -------- Step 5: embed videos in slides (if no videos, bot should no-op) --------
        final_results = []
        ...
        with coly:
            if st.button("清除本檔斷點資料（強制重跑）", use_container_width=True):
                # delete per-file state artifacts but keep workspace folder
                try:
        ...
        if auto_clean:
            # NOTE: do not delete resume files by default; only temp media
            # We'll clean temp media folder only
            try:
...
# Category options from sheet (do not block UI if fails)
...
with st.container():
    ...
    if input_method == "本地檔案":
        ...
        if uploaded_file:
            ...
            # write source first, then compute fingerprint
    ...
    if file_name_for_logic and os.path.exists(source_path):
        ...
        if st.session_state.current_file_name != file_name_for_logic:
            ...
            try:
                ...
                # If no jobs yet, auto-create one job spanning all pages (but keep fields)
            ...
            # Same file (user re-uploaded) => keep split_jobs as-is, only refresh source.pptx content
            # Recompute fingerprint for resume match
            pass
...
if st.session_state.current_file_name:
    ...
    with st.container():
        ...
        # Render each job card (FULL FIELDS MUST STAY)
        ...
        # Save history every render pass
        save_history(st.session_state.current_file_name, st.session_state.split_jobs)
...
if st.session_state.current_file_name:
    with st.container():
        ...
        # Resume indicator (based on fingerprint)
        src_fp = file_fingerprint(os.path.join(WORK_DIR, "source.pptx")) if os.path.exists(os.path.join(WORK_DIR, "source.pptx")) else ""
        can_resume, info = detect_resume(st.session_state.current_file_name, src_fp) if src_fp else (False, {})
        if can_resume:
        ...
        if st.button("執行自動化排程", type="primary", use_container_width=True):
            if not st.session_state.split_jobs:
                ...
                errs = validate_jobs(st.session_state.split_jobs, st.session_state.ppt_meta.get("total_slides", 0))
                if errs:
                    for e in errs:
                        st.error(e)
                    ...
                    # During execution, auto scroll to bottom so user sees progress
                    scroll_to_bottom()
