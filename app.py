# Version: v1.4 (Debug Mode)
# Update Log:
# 1. ADDED: Global Try-Except block to catch crashes and show them on UI.
# 2. UI: Layout adjustments from v1.3 included.
...
# 這裡加入 try-except 避免因找不到模組而直接白屏
try:
    from ppt_processor import PPTAutomationBot
except ImportError:
    st.error("❌ 找不到 `ppt_processor.py` 檔案，請確認該檔案已上傳至同一目錄。")
    st.stop()
...
# 捕捉並顯示全域錯誤，而非白屏 Oh No
except Exception as e:
    st.error("程式發生未預期的錯誤，詳細資訊如下：")
    st.code(traceback.format_exc())
